...
class LikeServiceTest(
    ...
    given("좋아요 생성시") {
        val saved = postRepository.save(Post("whahn", "title", "content"))
        When("인풋이 정상적으로 들어오면") {
            val likeId = likeService.createLike(1L, "whahn")
            then("좋아요가 정상적으로 생성됨을 확인한다") {
                val like = likeRepository.findByIdOrNull(likeId)
                like shouldNotBe null
                like?.createdBy shouldBe "whahn"
            }
        }
        When("게시글이 존재하지 않으면") {
            then("존재하지 않는 게시글 예외가 발생한다") {
                shouldThrow<PostNotFoundException> {
                    likeService.createLike(9999L, "whahn")
                }
            }
        }
    }
